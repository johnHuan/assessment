...
del_temp_file(config_dict["temp_file_planning"])  # 删除临时文件夹中现有数据
Planning_Preparation(config_dict)  # 数据预处理
Fire_SNC(config_dict)  # 消防服务能力评估
Sanitation_SNC(config_dict)  # 环卫服务能力评估

# 将计算完成的地块文件复制到结果文件夹
arcpy.CopyFeatures_management(config_dict["polygons_for_planning"],
                              os.path.join(config_dict["result_file"], "planning_result.shp"))

print("计算结果保存在：")
print(os.path.join(config_dict["result_file"], "planning_result.shp"))
